        ...
        <div class="wizard-box">
            ...
            <div class="wizard-step" id="wizardStep2">
                ...
                <div id="wizEndpointBlock">
                    ...
                    <input id="wizEndpoint" type="text" value="https://ark.cn-beijing.volces.com/api/coding/v3">
                    <div class="hint" data-i18n="endpointHint">完整的 Base URL，例如 https://ark.cn-beijing.volces.com/api/coding/v3</div>
                </div>
                ...
                <div id="wizModelSelectBlock">
                    ...
                    <select id="wizModelSelect">
                        <option value="ark-code-latest" selected>火山引擎 Ark Code（推荐）</option>
                        <option value="doubao-seed-code-preview-251028">字节 Doubao Seed Code</option>
                        <option value="kimi-k2-5-260127">月之暗面 Kimi K2.5</option>
                        <option value="glm-4-7-251222">智谱 GLM 4.7</option>
        ...
        <div class="settings-panel" id="settings-panel">
            ...
            <div class="card">
                ...
                <div style="font-size: 14px; color: #8b949e; line-height: 1.8;">
                    ...
                    <div>
                        ...
                        <input id="custom-endpoint" type="text" value="https://ark.cn-beijing.volces.com/api/coding/v3" oninput="saveEndpoint()" style="width: 100%; margin-top: 8px; padding: 8px 12px; background: #0d1117; border: 1px solid #30363d; border-radius: 6px; color: #e6edf3; font-size: 14px;">
                    </div>
            ...
            <div class="card">
                ...
                <div style="font-size: 13px; color: #8b949e; line-height: 1.8;">
                    <div id="storage-status">📦 localStorage状态: 检查中...</div>
                    <div id="ws-status">🔌 WebSocket状态: 未连接</div>
                    <div id="session-count">📋 会话数量: 0</div>
                    <div id="browser-info">🌐 浏览器信息: -</div>
                    <div style="margin-top: 12px;">
                        <button onclick="runDiagnostic()" style="background: #58a6ff; color: white; border: none; border-radius: 6px; padding: 6px 12px; cursor: pointer; font-size: 12px;">
                            运行诊断
                        </button>
                        <button onclick="exportHistory()" style="background: #238636; color: white; border: none; border-radius: 6px; padding: 6px 12px; cursor: pointer; font-size: 12px; margin-left: 8px;">
                            导出历史
                        </button>
                    </div>
                </div>
    ...
    <script>
        ...
        const translations = {
            zh: {
                ...
                configUpdated: '配置已更新',
                apiKeyLabel: 'API Key',
                endpointHint: '完整的 Base URL，例如 https://ark.cn-beijing.volces.com/api/coding/v3',
                modelHint: '输入你使用的模型名称',
                useVolcApiKey: '将使用 volc_api.txt 中的配置',
                useDeepSeekApiKey: '将使用 API.txt 中的配置',
                cardAbout: '📦 关于',
                versionLabel: '版本:',
                authorLabel: '作者:',
            ...
            en: {
                ...
                apiKeyLabel: 'API Key',
                endpointHint: 'Full Base URL, e.g. https://ark.cn-beijing.volces.com/api/coding/v3',
                modelHint: 'Enter the model name you use',
                useVolcApiKey: 'Will use volc_api.txt config',
                useDeepSeekApiKey: 'Will use API.txt config',
                cardAbout: '📦 About',
        ...
        function onWizProviderChange() {
            ...
            if (provider === 'deepseek' || provider === 'volcengine') {
                ...
                endpointInput.value = provider === 'volcengine' 
                    ? 'https://ark.cn-beijing.volces.com/api/coding/v3' 
                    : 'https://api.deepseek.com/v1';
                apiKeyInput.disabled = true;
                const hintKey = provider === 'volcengine' ? 'useVolcApiKey' : 'useDeepSeekApiKey';
                apiKeyInput.placeholder = '✔️ ' + translations[currentLang][hintKey];
                apiKeyHint.innerHTML = '✔️ ' + translations[currentLang][hintKey];
        ...
        function wizardFinish() {
            ...
            if (provider === 'deepseek' || provider === 'volcengine') {
                ...
                endpoint = document.getElementById('wizEndpoint').value.trim() 
                    || (provider === 'volcengine' ? 'https://ark.cn-beijing.volces.com/api/coding/v3' : 'https://api.deepseek.com/v1');
            } else {
        ...
        function saveProviderSetting() {
            ...
            localStorage.setItem('pyclaw_provider', provider);
            // 发送更新
            const model = (provider === 'deepseek' || provider === 'volcengine') 
        ...
        function loadSavedConfig() {
            ...
            if (configured) {
                onProviderChange();
                if ((provider === 'deepseek' || provider === 'volcengine') && model) {
                    document.getElementById('model-select').value = model;
        ...
        function runDiagnostic() {
            ...
            let storageOk = false;
            try {
                localStorage.setItem('pyclaw_test', 'test');
                localStorage.removeItem('pyclaw_test');
                storageOk = true;
                document.getElementById('storage-status').innerHTML = '📦 localStorage状态: ✅ 正常 (' + (localStorage.length / 1024).toFixed(2) + ' KB)';
            } catch(e) {
                document.getElementById('storage-status').innerHTML = '📦 localStorage状态: ❌ 失败 - ' + e.message;
            }
            
            // 检查WebSocket状态
            const wsStatus = ws ? (ws.readyState === WebSocket.OPEN ? '✅ 已连接' : '🔄 连接中/断开') : '❌ 未初始化';
            document.getElementById('ws-status').innerHTML = '🔌 WebSocket状态: ' + wsStatus;
            
            // 统计会话数量
            const sessions = JSON.parse(localStorage.getItem('pyclaw_sessions') || '[]');
            let totalMsgs = 0;
            sessions.forEach(sid => {
                const h = JSON.parse(localStorage.getItem('pyclaw_history_' + sid) || '[]');
                totalMsgs += h.length;
            });
            document.getElementById('session-count').innerHTML = '📋 会话数量: ' + sessions.length + ' | 消息总数: ' + totalMsgs;
            
            // 浏览器信息
            const browserInfo = navigator.userAgent.match(/(Chrome|Firefox|Safari|Edge)\/([0-9.]+)/)?.[0] || '未知浏览器';
            document.getElementById('browser-info').innerHTML = '🌐 浏览器信息: ' + browserInfo + ' (' + (navigator.platform || navigator.userAgentData?.platform || '未知平台') + ')';
            
            // 显示详细存储信息
            if (storageOk && confirm('是否查看详细存储信息？')) {
                let info = '=== PyClaw 存储信息 ===\n\n';
        ...
        function exportHistory() {
            ...
            a.href = url;
            a.download = 'pyclaw_history_' + new Date().toISOString().slice(0,10) + '.json';
            a.click();
