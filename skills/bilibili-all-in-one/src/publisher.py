...
DYNAMIC_CREATE_URL = f"{API_BASE}/x/dynamic/create"
...
class BilibiliPublisher:
    ...
    async def publish_dynamic(
        ...
        """Publish a text-only dynamic to Bilibili.

        Args:
            content: The text content of the dynamic.

        Returns:
            Publish result.
        """
        if not content or len(content.strip()) == 0:
            return {"success": False, "message": "Dynamic content cannot be empty"}
        
        if len(content) > 2000:
            return {"success": False, "message": "Dynamic content must be 2000 characters or less"}

        # Create dynamic request - modern Bilibili API
        import json
        # Build the content structure for text dynamic
        content_obj = {
            "content": content,
            "at_control": "[]",
            "location": {}
        ...
        async with self._get_client() as client:
            resp = await client.post(DYNAMIC_CREATE_URL, data=payload)
            try:
                data = resp.json()
        ...
        if data.get("code") != 0:
            return {
                "success": False, 
                "message": data.get("message", "Dynamic publish failed"),
                "code": data.get("code")
            }
        ...
        return {
            "success": True,
            "dynamic_id": result_data.get("dynamic_id"),
            "message": "Dynamic published successfully",
        }
