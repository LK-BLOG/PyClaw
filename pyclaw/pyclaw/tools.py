...
class FileReadTool:
    ...
    async def execute(self, params: Dict[str, Any]) -> ToolResult:
        ...
        try:
            ...
            if sys.platform.startswith('win') and '/' in file_path:
...
class ListDirTool:
    ...
    async def execute(self, params: Dict[str, Any]) -> ToolResult:
        ...
        try:
            ...
            if sys.platform.startswith('win') and '/' in dir_path:
...
@dataclass
class ExecTool:
    """命令执行工具 - 执行系统 shell 命令"""
    
    @property
    def definition(self) -> ToolDefinition:
        return ToolDefinition(
            name="exec_command",
            description="执行系统 shell 命令，返回输出结果。自动适应 Windows/Linux/Mac 系统",
            parameters={
                "type": "object",
                "properties": {
                    "command": {
                        "type": "string",
                        "description": "要执行的 shell 命令，Windows 可用 'dir'、'echo' 等，Linux/Mac 可用 'ls -la' 等"
                    },
                    "timeout": {
                        "type": "integer",
