#!/bin/bash
# 使用Python处理JSON，避免转义问题
python3 << 'EOF'
import requests
import sys

tunnel_url = "https://warned-baker-reproductive-invite.trycloudflare.com"
content = f"""个人网站更新地址👇
{tunnel_url}
...
headers = {'User-Agent': 'Mozilla/5.0', 'Referer': 'https://t.bilibili.com/', 'Content-Type': 'application/json'}
payload = {"dyn_req": {"content": {"contents": [{"raw_text": content, "type": 1, "biz_id": "", "size": None}]}, "scene": 4, "attach_card": None, "meta": {"app_meta": {"appid": 1, "mobi_app": "web", "version": "1.0.0"}}}}
...
if result.get('code') == 0:
    print(f"Dynamic ID: {result.get('data', {}).get('dyn_id')}")
EOF
